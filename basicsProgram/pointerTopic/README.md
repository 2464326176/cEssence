...
- 指针p的类型必须与变量value类型一致；int value和int *p；
- 指针就是地址，出现指针的地方都可以用地址代替，变量的指针就是变量的地址，指针变量就是地址变量
- **指针**就是地址本身，value的地址为62fe00，62fe00就是变量的指针；
- **指针变量**用来存放地址的变量，指针变量的值是一个地址；
- 指向：通过地址能找到具有该地址的对象，对于指针变量来说，把谁的地址存放在指针变量中，就说该指针指向谁；
...
在程序中定义一个函数，在编译的时候 编译系统为函数代码分配一段存储空间，被称为存储空间的起始地址（入口地址），也被称为函数的指针，其本质是一个指针；
...
定义一个函数指针类型，表示指向返回值为int，且同时带有两个int参数的函数指针类型；可以用这种类型定义函数指针调用相同类型的函数；

## 3.2 指针函数

返回指针值的函数，类型名 *函数名（参数表列）
