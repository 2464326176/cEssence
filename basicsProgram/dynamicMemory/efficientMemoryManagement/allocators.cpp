/*
 * @Author: yh
 * @Date: 2023-04-03 23:59:21
 * @LastEditTime: 2023-04-04 00:02:13
 * @Description:
 * @FilePath: \cEssence\basicsProgram\dynamicMemory\efficientMemoryManagement\allocators.cpp
 */
...
template <class T> void swap(T *a, int index1, int index2) {
...
template <typename T> __int64 reverse_array() {
    ...
    for (int i = 0; i < repeat; ++i) {
        time += time_call([&] { parallel_for(0, size / 2, [&](int index) { swap(a, index, size - index - 1); }); });
    }
...
int wmain() {
    ...
    // new_delete
    wcout << L"Took " << reverse_array<new_delete>() << " ms with new/delete." << endl;

    // malloc_free
    wcout << L"Took " << reverse_array<malloc_free>() << " ms with malloc/free." << endl;

    // Alloc_Free
    wcout << L"Took " << reverse_array<Alloc_Free>() << " ms with Alloc/Free." << endl;
}